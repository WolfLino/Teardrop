...
void teardrop(float x, float y, float comp, float anguloAbertura, float rotacao) {
...
void teardropHard(float posX, float posY) {
	float x[8] = {};
	float y[8] = {};

	for (int i = 0; i < 8; i++) {
		x[i] = posX + (40 * coss(45 * i));
		y[i] = posY + (40 * seno(45 * i));
	}

	for (int i = 0; i < 8; i++) {
		teardrop(x[i], y[i], 100, 40, (-92) + 45 * i);
	}
}
...
void desenha(void) {
	...
	// Desenha o teardrop
	//teardrop(tdPos.x, tdPos.y, 100, 45, 90);
	teardropHard(tdPos.x, tdPos.y);
